...
public class SolaceConsumer {
    ...
    public void startConsumingFromTopic(String topicName) throws JCSMPException {
        ...
        connection.getSession().addSubscription(topic);
        consumer.start();
    }
    ...
    public void startConsumingFromQueue(String queueName) throws JCSMPException {
        ...
        consumer = connection.getSession().getMessageConsumer(new MessageListener());
        FlowReceiver flowReceiver = connection.getSession().createFlow(
                new MessageListener(),
                new ConsumerFlowProperties().setEndpoint(queue)
        );
        flowReceiver.start();
    }
    ...
    private class MessageListener implements XMLMessageListener {
        ...
        private Message createMessage(BytesXMLMessage message) {
            if (message instanceof com.solacesystems.jcsmp.TextMessage) {
                return new org.example.message.TextMessage(((com.solacesystems.jcsmp.TextMessage) message).getText());
            } else {
                return new BinaryMessage(message.getBytes());
            }
        }
    }
